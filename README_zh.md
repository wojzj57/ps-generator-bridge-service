...
发布的 Windows x64 generator runtime 是独立包：JavaScript runtime 依赖会被
bundle，sharp 及其原生 vendor payload 保留在包内私有目录。
...
| `PS_BRIDGE_PLUGINS`            | 在其他来源前加载、以平台分隔符连接的插件绝对路径。       |
...
结构化运行参数（例如 `port`、`plugins`、`pluginsDir`）应通过 `PluginConfig` 传入；
环境变量只作为部署覆盖项。插件来源依次为 `PS_BRIDGE_PLUGINS`、
`PluginConfig.plugins`、`pluginsDir` / `PS_BRIDGE_PLUGINS_DIR` 中按名称排序的子目录。
