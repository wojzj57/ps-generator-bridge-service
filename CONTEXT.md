...
**clientId（客户端连接 id）**:
标识一个**逻辑客户端会话**的稳定身份，**跨重连保持**——只能由 server 生成并在 `connected` 中回传，[Connection](#) 记住它，重连时经 `?resume=` 回传。缺失、未知、过期或格式错误的 resume id 会创建新会话，不会报错。server 用它登记会话并将 [Event](#) 定向推送。
...
_Avoid_: [Transport](#)（更底层的单连接接缝）、PsBridgeClient（已被 Connection 取代 / 吸收）、socket

**LightBox Photoshop 入口**:
外部网页入口，用于在 bridge server 不健康时引导用户打开 Photoshop 相关能力。它不是 [Protocol](#) 能力，也不代表 sdk 能直接启动本机 Photoshop 进程。
_Avoid_: 远程启动 PS（容易暗示本机进程控制）、Connection 方法（它是独立 SDK helper）
...
**ConnectionSession（连接会话） / SessionStore（会话表）**:
server 侧用 ConnectionSession 表示一个由 [clientId](#) 标识的逻辑会话，内部持有当前 socket 和订阅；插件 handler 只看到不含 socket 的 `WsSession`。SessionStore 同时记录在线会话和意外断线后暂时可恢复的会话。新 socket 用 `?resume=` 接管时会关闭旧 socket，并立即释放旧订阅，由 SDK 重放订阅。显式 `close()` 立即删除会话；意外断线默认保留 30 分钟，到期才删除。
_Avoid_: [Registry](#)（那是装配接缝）、ClientStore（只表达在线 socket，忽略可恢复会话）、pool、connection registry
